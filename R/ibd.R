g=function(v,b,k,x,z)
{
  value=((v-1)^2)/(b*v*k*(k-1)-(b*x+z)*(k*v-v+k)+b*x*x+2*x*z+z)+1/(k*(b*x+z)-(b*x*x+2*x*z+z))  
  return(value)
}
##################################################################################################
gts=function(v,b,k)
...
################################################################
LIP=function(v,b,kvec,NNPo,N1,T,rownum,relaxed,binary=TRUE)
{
  if(missing(binary)) binary=TRUE
  ...
  types=rep("B", times=b)	
  sol=lp (direction = "max", obj, constr, dir, rhs,transpose.constraints = TRUE, all.bin=binary, use.rw=TRUE)
  if (sol[[28]]==0) 
  {		
    row=sol[[12]]
    dim(row)=c(1,b)
...
###############################################################################
btibgen=function(v,b,k,NNPo,ntrial,pbar)
{
  R=matrix(0,v,v);
  for (i in 1:v) R[i,i]=NNPo[i,i]
  C=R-NNPo/k	
  dt=det(C+matrix(1/v,v,v))
  dt=round(dt,digits=6)
  if (dt!=0)  connected=1 else connected=0
  ...
  {	
    ...
    while(trial<=ntrial & success==0)
    {
      ...
      while (i<=v & decision==0)
      {
        ...
        if (i<v) N1=LIP(v,b,kvec,NNPo,N1,T,i,relaxed) else N1=LIP(v,b,kvec,NNPo,N1,T,i,relaxed,binary=FALSE)
        if (nrow(N1)<i ) 
        {
          temp=detect(v,b,kvec,NNPo,N1,T,relaxed)
          rows=temp[[1]]
...
btib1=function(v,b,r,r0,k,lambda,lambda0)
{
  ...
  if (is.matrix(design)) 
  {
    ...
    NNP=N%*%t(N)	
    Aeff=gts(v,b,k)*lambda0*(lambda0+v*lambda)/(lambda0+lambda)
...
btib=function(v,b,r,r0,k,lambda,lambda0,ntrial=5,pbar=FALSE)
{
  ...
  N=btibgen((v+1),b,k,NNPo,ntrial,pbar)
  if (is.matrix(N))
  {	
    NNP=N%*%t(N)
    design=N_to_design(N)
...
ibdtvc=function(v1,v2,b,k,NNPo,ntrial=5,pbar=FALSE)
{
  ...
  {	
    NNP=N%*%t(N)
    design=N_to_design(N)
    result=list(v1=v1,v2=v2,b=b,k=k,design=design,N=N,NNP=NNP)
  } else result="design not found"	
  return(result)	
}
##################################################################################################
